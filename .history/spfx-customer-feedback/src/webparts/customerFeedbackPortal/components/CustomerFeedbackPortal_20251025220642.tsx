...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  private async ensureListExistsOnTarget(listTitle: string, targetWebUrl: string): Promise<boolean> {
    try {
      const lists = await this.props.sp.web.getWeb(targetWebUrl).lists.filter(`Title eq '${listTitle.replace("'", "''")}'`).select('Title','Id')();
      return lists.length > 0;
    } catch (err) {
