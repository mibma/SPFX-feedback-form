...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  private async ensureListExistsOnTarget(listTitle: string, targetSp: any): Promise<boolean> {
    try {
      const lists = await targetSp.lists.filter(`Title eq '${listTitle.replace("'", "''")}'`).select('Title','Id')();
      return lists.length > 0;
    } catch (err) {
  ...
  private async handleSubmit(): Promise<void> {
    ...
    try {
      ...
      // Set the target web URL
      const targetWeb = targetSp.web.getWebUrlFromPageUrl(targetSiteUrl);
      
      // Check if list exists
      const listExists = await this.ensureListExistsOnTarget(listTitle, targetSp);
      if (!listExists) {
      ...
      await targetSp.web.lists.getByTitle(listTitle).items.add({
        Customer_name: name,
        Age: ageNum,
        Email: email,
        Rate: rate,
