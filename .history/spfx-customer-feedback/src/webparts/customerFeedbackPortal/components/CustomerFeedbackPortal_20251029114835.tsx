  ...
  return (
    <section className={`${styles.customerFeedbackPortal ?? ''} ${hasTeamsContext ? styles.teams ?? '' : ''}`}>
      ...
      <div className={styles.feedbackForm}>
        {message && (
          <div
            className={messageType === 'error' ? styles.messageError : styles.messageSuccess}
            role="status"
            aria-live="polite"
          >
            {message}
        ...
        <div className={styles.formGrid}>
          ...
          <div className={styles.field}>
            <Dropdown label="Rating (1-5)" selectedKey={rate} options={rateOptions} onChange={(_e, opt) => setRate(Number(opt?.key ?? 1))} />
          </div>
          <div className={`${styles.field} ${styles.span2}`}>
