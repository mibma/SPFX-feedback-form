...
const CustomerFeedbackPortal: React.FC<CustomerFeedbackPortalProps> = ({
  ...
  // On mount: attempt to fetch current user (Title and Email) with PnP.
  // Use the fetched values only if userDisplayName wasn't provided or email is missing.
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const sp = getSP();
        // select Title and Email from current user
        const current = await sp.web.currentUser.select('Title', 'Email')();
        if (current) {
          // If userDisplayName prop wasn't provided, prefer current.Title
          if (!userDisplayName && current.Title) {
            setName(current.Title);
          }
          // If email is available from current user, set it (override empty)
          if (current.Email) {
            setEmail(current.Email);
          }
        }
      } catch (err) {
        // silently ignore — we still allow the webpart to work with userDisplayName prop
    ...
    fetchCurrentUser();
    // if userDisplayName prop changes, keep name in sync
    if (userDisplayName) {
      setName(userDisplayName);
    }
    // We intentionally do not reset email when userDisplayName changes; fetchCurrentUser will set email if available.
