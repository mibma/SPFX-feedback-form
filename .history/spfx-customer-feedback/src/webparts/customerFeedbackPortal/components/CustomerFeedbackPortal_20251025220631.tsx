...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  private async handleSubmit(): Promise<void> {
    ...
    try {
      // Use the target site URL directly
      const targetWeb = this.props.sp.web.getWebUrlFromPageUrl(targetSiteUrl);
      
      // Check if list exists on target site
      const listExists = await this.ensureListExistsOnTarget(listTitle, targetSiteUrl);
      if (!listExists) {
      ...
      await this.props.sp.web.getWeb(targetSiteUrl).lists.getByTitle(listTitle).items.add({
        Customer_name: name,
        Age: ageNum,
        Email: email,
        Rate: rate,
