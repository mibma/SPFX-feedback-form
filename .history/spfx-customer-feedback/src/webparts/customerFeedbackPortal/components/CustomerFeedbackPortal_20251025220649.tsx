...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  private async ensureListExists(listTitle: string): Promise<boolean> {
    try {
      const lists = await this.props.sp.web.lists.filter(`Title eq '${listTitle.replace("'", "''")}'`).select('Title','Id')();
  ...
  private async ensureListExistsOnTarget(listTitle: string, targetSp: any, targetSiteUrl: string): Promise<boolean> {
    try {
      const lists = await targetSp.web.getWebUrlFromPageUrl(targetSiteUrl).lists.filter(`Title eq '${listTitle.replace("'", "''")}'`).select('Title','Id')();
      return lists.length > 0;
    } catch (err) {
      console.error('Error checking list existence on target site', err);
      return false;
    }
  }
  ...
  private async handleSubmit(): Promise<void> {
    ...
    // Connect to the specific SharePoint site and list
    const listTitle = 'Feedbacklist';
    const targetSiteUrl = 'https://mibmacloud.sharepoint.com/sites/CustomerFeedbackPortal';
    
    try {
      // Create a new SP instance for the target site
      const { spfi, SPFx } = await import('@pnp/sp');
      const targetSp = spfi().using(SPFx(this.props.sp.web._spfxContext));
      
      // Check if list exists on target site
      const listExists = await this.ensureListExistsOnTarget(listTitle, targetSp, targetSiteUrl);
      if (!listExists) {
        this.setState({ submitting: false, message: `${listTitle} not found on the target site. Please create it or contact admin.`, messageType: 'error' });
        return;
      }

      // Add item to the target list
      await targetSp.web.getWebUrlFromPageUrl(targetSiteUrl).lists.getByTitle(listTitle).items.add({
        Customer_name: name,
        Age: ageNum,
        Email: email,
        Rate: rate,
