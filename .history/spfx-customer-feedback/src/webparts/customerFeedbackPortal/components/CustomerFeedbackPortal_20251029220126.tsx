...
const CustomerFeedbackPortal: React.FC<CustomerFeedbackPortalProps> = ({
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const sp = getSP();
        // Use .get() on currentUser (cast to any to avoid TypeScript typing errors)
        const currentUser = await (sp as any).web.currentUser.get();
        if (currentUser) {
          // prefer the userDisplayName prop if provided, otherwise use Title from current user
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const spfxx = spfi().using(SPFx(this.context));
      // EMAIL
