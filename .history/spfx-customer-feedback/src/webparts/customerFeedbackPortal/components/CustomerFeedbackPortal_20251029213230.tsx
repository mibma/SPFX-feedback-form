...
import * as pnp from "@pnp/sp"; // used for new Web(...) pattern
...
interface CustomerFeedbackPortalProps {
  ...
  userDisplayName?: string; // optional prop you already use for welcome
  ctx?: any; // <--- pass the web part context here (this.context)
}
...
const CustomerFeedbackPortal: React.FC<CustomerFeedbackPortalProps> = ({
  ...
  // Fetch current user using the new Web(ctx.pageContext.site.absoluteUrl) pattern you supplied
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        if (!ctx || !ctx.pageContext || !ctx.pageContext.site) {
          // fall back to getSP() approach if ctx not provided
          const sp = getSP();
          const current = await (sp as any).web.currentUser.get();
          if (current) {
            if (!userDisplayName && current.Title) setName(current.Title);
            if (current.Email) setEmail(current.Email);
          }
          return;
        }

        // create Web instance for the site URL from ctx and call currentUser.get()
        const web = new ((pnp as any).Web)(ctx.pageContext.site.absoluteUrl);
        const currentUser = await web.currentUser.get();

        if (currentUser) {
          // prefer the userDisplayName prop for name if provided; otherwise use currentUser.Title
        ...
        // don't crash the app if fetching fails; log for debugging
        // eslint-disable-next-line no-console
        console.warn('Could not fetch current user with ctx.Web(...).currentUser.get():', err);

        // fallback: try sp.web.currentUser.get() if available (keeps behaviour resilient)
        try {
          const sp = getSP();
          const current = await (sp as any).web.currentUser.get();
          if (current) {
          ...
          // eslint-disable-next-line no-console
          console.warn('Fallback currentUser.get() also failed:', innerErr);
        }
      }
    };
    ...
    // keep name in sync if userDisplayName prop changes
    if (userDisplayName) {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    // basic validation: Name, Email, Service, Rating required
    ...
    try {
      ...
      // SERVICE (choice field) - try multiple likely internal names
      ...
      for (const n of serviceCandidates) {
        if (hasField(n)) {
          // For SharePoint Choice fields, posting the text value is usually fine
      ...
      // reset fields (keep name/email since they are user context-driven)
      setRate(0);
      setComments('');
      setService('');
    } catch (err: any) {
