...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  // explicit return type
  private handleSubmit kommunista = async (): Promise<void> => {
    ...
    try {
      ...
      const fieldsArray = await this.props.sp.web.lists.getByTitle(listTitle).fields.select('InternalName', 'Title')();
      console.log('Available fields:', fieldsArray);
      
      // Helper function to check if a field exists (array.find not available)
      const hasField = (fieldName: string): boolean => {
        for (let i = 0; i < fieldsArray.length; i++) {
          if ((fieldsArray[i] as any).InternalName === fieldName) {
            return true;
          }
        }
        return false;
      ...
      } else if (hasField('customer_name')) {
        itemData.customer_name = name;
      }
      
      if (hasField('Age')) {
        itemData.Age = ageNum;
      } else if (hasField('age')) {
        itemData.age = ageNum;
      }
      ...
      if (hasField('Rate')) {
        itemData.Rate = rate;
      } else if (hasField('rate')) {
        itemData.rate = rate;
      } else if (hasField('Rating')) {
        itemData.Rating = rate;
      }
      
      if (comments && hasField('Comments')) {
        itemData.Comments = comments;
      } else if (comments && hasField('comments')) {
        itemData.comments = comments;
      }
