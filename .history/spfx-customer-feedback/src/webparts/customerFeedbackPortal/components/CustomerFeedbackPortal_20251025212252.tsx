...
export default class CustomerFeedbackPortal extends React.Component<ICustomerFeedbackPortalProps, ICustomerFeedbackPortalState> {
  ...
  private async ensureListExists(listTitle: string): Promise<boolean> {
    try {
      const lists = await this.props.sp.web.lists.filter(`Title eq '${listTitle.replace("'", "''")}'`).select('Title','Id').get();
  ...
  private async handleSubmit(): Promise<void> {
    ...
    // Ensure the list exists on the current web
    const listTitle = 'Feedbacklist'; // use exact title as in your site
    const listExists = await this.ensureListExists(listTitle);
    if (!listExists) {
      this.setState({ submitting: false, message: `${listTitle} not found on this site. Please create it or contact admin.`, messageType: 'error' });
      return;
    }
  ...
  // small test function to debug list discovery (explicit return type)
  private async debugListTitles(): Promise<void> {
    try {
      const lists = await this.props.sp.web.lists.select('Title','Id').get();
      console.log('Lists in this web:', lists.map(l => l.Title));
    } catch (err) {
      console.error('debugListTitles error', err);
    }
  }
      ...
      <section className={`${styles.customerFeedbackPortal} ${hasTeamsContext ? styles.teams : ''}`}>
        ...
        <div className={styles.feedbackForm}>
          ...
          <div style={{ marginTop: 12 }}>
            <PrimaryButton text={submitting ? "Submitting..." : "Submit Feedback"} onClick={async () => await this.handleSubmit()} disabled={submitting} />
            <PrimaryButton text="Debug Lists (console)" onClick={async () => await this.debugListTitles()} style={{ marginLeft: 8 }} />
          </div>
        </div>
      </section>
    );
